...
#define BT_RC_RFCOMM_CHANNEL 21
...
static atomic_int profile_status = BT_REMOTE_CONTROL_DISABLED;
...
static DBusHandlerResult profile_message_handler(DBusConnection * c,
    ...
    if (strcmp(member, "Release") == 0) {
        registered = false;
        atomic_store(&profile_status, BT_REMOTE_CONTROL_WAITING);
    ...
    /* libdbus gives us an owned descriptor for DBUS_TYPE_UNIX_FD. Keep that
     * descriptor for the worker; duplicating it here would leak the original. */
    int fd_flags = fcntl(received_fd, F_GETFD);
    int status_flags = fcntl(received_fd, F_GETFL);
    if (received_fd < 0 || fd_flags < 0 || status_flags < 0 ||
        fcntl(received_fd, F_SETFD, fd_flags | FD_CLOEXEC) < 0 ||
        fcntl(received_fd, F_SETFL, status_flags & ~O_NONBLOCK) < 0) {
        if (received_fd >= 0) close(received_fd);
        send_error(c, msg, "org.bluez.Error.Rejected", "Could not retain RFCOMM connection");
        return DBUS_HANDLER_RESULT_HANDLED;
    }
    struct timeval io_timeout = { .tv_sec = 5, .tv_usec = 0 };
    if (setsockopt(received_fd, SOL_SOCKET, SO_RCVTIMEO, &io_timeout, sizeof(io_timeout)) != 0 ||
        setsockopt(received_fd, SOL_SOCKET, SO_SNDTIMEO, &io_timeout, sizeof(io_timeout)) != 0) {
        close(received_fd);
...
static bool append_option_uint16(DBusMessageIter * dict, const char * key,
                                 dbus_uint16_t value) {
    DBusMessageIter entry, variant;
    if (!dbus_message_iter_open_container(dict, DBUS_TYPE_DICT_ENTRY, NULL, &entry) ||
        !dbus_message_iter_append_basic(&entry, DBUS_TYPE_STRING, &key) ||
...
/* Give this vendor UUID an explicit RFCOMM channel so BlueZ publishes a usable
 * RFCOMM protocol descriptor in SDP. Android discovers that channel by UUID.
 * Require authentication so BlueZ completes pairing before NewConnection. */
static bool register_profile(bool register_it) {
    ...
    if (!dbus_message_iter_append_basic(&iter, DBUS_TYPE_OBJECT_PATH, &path) ||
        (register_it && !dbus_message_iter_append_basic(&iter, DBUS_TYPE_STRING, &uuid))) {
    ...
    if (register_it) {
        ...
        bool ok = append_option_uint16(&dict, "Channel", BT_RC_RFCOMM_CHANNEL) &&
                  append_option_string(&dict, "Name", BT_RC_NAME) &&
    ...
    if (!reply) {
        ...
        dbus_error_free(&err);
        if (register_it) atomic_store(&profile_status, BT_REMOTE_CONTROL_FAILED);
        return false;
    }
    dbus_message_unref(reply);
    dbus_error_free(&err);
    registered = register_it;
    atomic_store(&profile_status, register_it ? BT_REMOTE_CONTROL_READY : BT_REMOTE_CONTROL_WAITING);
...
static void * service_thread_func(void * unused) {
    ...
    while (atomic_load(&enabled)) {
        if (!connection) {
            ...
            if (!dbus_connection_register_object_path(connection, BT_RC_OBJECT_PATH, &profile_vtable, NULL)) {
                ...
                connection = NULL;
                atomic_store(&profile_status, BT_REMOTE_CONTROL_FAILED);
                usleep(BT_RC_RETRY_MS * 1000);
                continue;
            }
            bluez_present = false;
    ...
    bluez_owner[0] = '\0';
    registered = false;
    atomic_store(&profile_status, BT_REMOTE_CONTROL_DISABLED);
    return NULL;
}
...
bt_remote_control_status_t bt_remote_control_status(void) {
    return (bt_remote_control_status_t) atomic_load(&profile_status);
}

bool bt_remote_control_is_advertising(void) {
    return bt_remote_control_status() == BT_REMOTE_CONTROL_READY;
}
