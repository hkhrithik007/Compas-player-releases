 ...
 *    completely separate concern from compas_player below, and not
 *    something this update mechanism touches.
 *  - "compas_player": a build of THIS SAME app the user dropped on the
...
#define LEGACY_SD_STOCK_PLAYER_PATH LEGACY_SD_ALT_DIR "/hiby_player"
#define LEGACY_SD_UPDATE_PLAYER_PATH LEGACY_SD_ALT_DIR "/open_hiby_player"

/* Same /usr/data root as settings.c's own .compas/settings.txt -- the one
...
typedef struct {
    ...
    /* Path of the detected stock player, allowing existing cards that use
     * the legacy hidden directory to keep working during migration. */
    const char *sd_stock_path;
